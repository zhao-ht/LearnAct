...
from aux_func.aux_func import execute, parse_python_func
from backbone.gpt import num_tokens_from_messages


class code_as_policy:
    ...
    def make_prompt(
        ...
        query = ""
        query += (
            self.split["instruction"][0] + instruction + self.split["instruction"][-1]
        )
        ...
        if len(examples) > 0:
            if self.hier:
                query += "\nYou can define functions in Python to solve the task. Note that the provided function in the example is not available, and you need to define your own functions."
    ...
    def run(self, action_space=None, world_model=None, plan=None, grounding_fn=None):
        ...
        human_do = False
        if not human_do:
            action, _ = self.backbone_func(messages)

        code = parse_python_func(action)
    ...
    def __call__(self, problem_input, **kwargs):
        ...
        env, state, reward, done, infos = self.execute_action(env, action_type, action)
        ...
        print(
            "Step {}: Reward: {}, Is done: {}".format(step_id, round(reward, 3), done)
        )
        ...
        if done:
            ...
            return {
                ...
                "gen": {
                    "reward": reward,
                    "steps": 1,
                    "grounding_acc": grounding_acc_count / 1,
